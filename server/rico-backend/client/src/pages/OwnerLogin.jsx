  ...
  async function handleLogin(e) {
    ...
    try {
      const res = await fetch('/auth/login', {
        ...
        body: JSON.stringify({ email, password, app: 'owner' }),
      });
      if (!res.ok) {
        setStatus({ type: 'error', message: 'البريد الإلكتروني أو كلمة المرور غير صحيحة.' });
        return;
      }
  ...
  async function handleForgot(e) {
    ...
    try {
      const res = await fetch('/auth/forgot-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, app: 'owner' }),
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface px-4" dir="rtl">
      <div className="bg-surface-container-lowest rounded-3xl shadow-lg p-8 max-w-sm w-full space-y-6">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <h1 className="text-xl font-bold text-primary">لوحة تحكم ريكو</h1>
          <p className="text-sm text-on-surface-variant">
            {mode === 'login' ? 'لفريق ريكو الداخلي فقط.' : 'أدخل بريدك الإلكتروني لإعادة تعيين كلمة المرور.'}
          </p>
        </div>
